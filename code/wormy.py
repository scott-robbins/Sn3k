...
def runGame():
    ...
    if os.path.isfile('high_score.txt'):
        high_score = int(utils.swap('high_score.txt', False).pop().split('\t')[0])
    else:
    ...
    while True: # main game loop
        ...
        if wormCoords[HEAD]['x'] == -1 or wormCoords[HEAD]['x'] == CELLWIDTH or wormCoords[HEAD]['y'] == -1 or wormCoords[HEAD]['y'] == CELLHEIGHT:
            if score > high_score:
                ...
                open('high_score.txt', 'wb').write('%d\t%s' % (score, user))
            return # game over
...
def drawScore(score):
    # Keep Track of HighScore
    if os.path.isfile('high_score.txt'):
        highest = int(utils.swap('high_score.txt', False).pop().split('\t')[0])
    else:
        open('high_score.txt', 'wb').write('0\tNo One')
        highest = 0
